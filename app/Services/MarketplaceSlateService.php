...
class MarketplaceSlateService
{
    ...
    {
        $globalMean = DB::table('place_details_cache')->avg('rating') ?? 4.0;
        ...
        $maxDistance = $providers->max(fn ($p) => $this->distanceKm($metro, $p)) ?: 1;

        return $providers
            ->map(function ($provider) use ($metro, $globalMean, $maxDistance) {
                $distanceKm = $this->distanceKm($metro, $provider);
                $proximityNorm = 1 - min($distanceKm / max($maxDistance, 1), 1);
